...
#[cfg(test)]
mod tests {
	use crate::{hmac, x448::{PrivateKeyX448, PublicKeyX448, KeyPairX448}, chain_key::{ChainKey}, root_key::RootKey, ed448::{PrivateKeyEd448}, signed_public_key::SignedPublicKey, signed_key_pair::SignedKeyPairX448};
	use super::MasterKey;

	// TODO: move to test-utils or alike
	...
	fn expected_chain() -> ChainKey {
		ChainKey::new(hmac::Key::from(b"\xc2\x8d\x58\xbd\x31\x7e\x21\xbf\xd1\x20\x4a\x11\xed\xf1\x14\x20\xe0\x79\x13\xe0\x55\xac\x3f\xc0\xd6\x1a\xd0\x77\x17\x43\x96\x7f"), 0)
	}
	...
	#[test]
	fn test_master_bob() {
		let key = MasterKey::bob(
			&KeyPairX448::new(bob_identity_x448_priv(), bob_identity_x448_pub()),
		 &SignedKeyPairX448::new(bob_signed_prekey_x448_priv(), bob_signed_prekey()), 
		 &KeyPairX448::new(bob_prekey_x448_priv(), bob_prekey_x448_pub()), 
	...
	fn test_derive() {
		let key = MasterKey::bob(
		 ...
		 let derived = MasterKey::derive(&key.root_key, &KeyPairX448::new(bob_prekey_x448_priv(), bob_prekey_x448_pub()), &alice_ephemeral_x448_pub());

		 assert_eq!(derived.chain_key().key().as_bytes(), b"\xa4\x7d\xbf\xdf\x34\x5b\x17\x38\x32\xbb\x2e\x95\x1f\xe5\x11\xed\x8d\xcd\xc5\x2b\x99\x89\x23\x6b\xef\x18\x72\x50\xa7\xb9\x74\xf3");
		 assert_eq!(derived.root_key().as_bytes(), b"\xf9\xb4\x95\x5c\xaa\xf4\x19\x31\x51\x77\x89\x4a\x55\x4f\x15\xab\xdd\xcc\x3c\xfd\x69\x04\x51\x0b\x17\x5f\xfb\x5e\xd6\xb2\xa8\xab");
		 assert_eq!(derived.chain_key().counter(), 0);
	}
}