...
pub trait Sessions {
	...
	fn get_active(&self, nid: &str) -> Option<Session>;
	/// Returns any session, whether active or receive_only
	fn get_by_id(&self, id: u64) -> Option<Session>;

	/// Clears active and receive_only sessions, if any
	fn clear_all(&self, nid: &str); // TODO: result?
	fn save(&self, session: &Session, nid: &str, id: u64, receive_only: bool); // TODO: introduce result
}
...
pub trait Prekeys {
	fn get(&self, id: u64) -> Option<Prekey>; // TODO: use Result instead; a separate consume?
	fn get_signed(&self, id: u64) -> Option<SignedKeyPair>; // TODO: use Result instead and handle errors: locked vs not found
...
impl Cryptor {
	pub async fn decrypt(&mut self, mac: &[u8], nid: &str, my_nid: &str) -> Result<Decrypted, Error> {
		...
		if let Some(ref kex) = mac.body().key_exchange {
			...
			if let Some(session) = self.sessions.get_by_id(kex.id()) {
				...
				let identity = self.identities.get_my_identity().ok_or(Error::NoIdentityFound)?;
				let ntru_identity = self.identities.get_my_ntru_identity().ok_or(Error::NoNtruIdentityFound)?;

				let signed_prekey = self.prekeys.get_signed(kex.signed_prekey_id).ok_or(Error::NoSignedPrekeyFound(kex.signed_prekey_id))?;
				let Prekey { key_x448, key_ntru, .. } = self.prekeys.get(kex.x448_prekey_id).ok_or(Error::NoPrekeyFound(kex.x448_prekey_id))?;
				...
				if kex.force_reset {
					self.sessions.clear_all(nid);

					return self.decrypt_with_session(session, mac, nid);
				} else {
					// do I have any other session for this nid?
					if let Some(_) = self.sessions.get_active(nid) {
	...
	fn decrypt_with_session(&self, mut session: Session, mac: AxolotlMac, nid: &str) -> Result<Decrypted, Error> {
		if let Ok(msg) = session.decrypt(&mac) {
			...
			self.sessions.save(&session, nid, session.id(), session.receive_only());
