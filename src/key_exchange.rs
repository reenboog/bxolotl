use crate::{proto, x448::PublicKeyX448, ntru::{NtruEncryptedKey, PublicKeyNtru}, ed448::PublicKeyEd448, id};
...
impl KeyExchange {
	pub fn derive_id(identity: &PublicKeyX448, prekey_id: u64) -> u64 {
		id::from_bytes(&[&identity.as_bytes()[..], &prekey_id.to_le_bytes()].concat())
	}

	pub fn id(&self) -> u64 {
		Self::derive_id(&self.x448_identity, self.x448_prekey_id)
	}
}
...
mod tests {
	...
	fn test_derive_id() {
		...
		assert_eq!(KeyExchange::derive_id(&alice_identity, bob_prekey.id()), 17735092266174708247);
	}
}